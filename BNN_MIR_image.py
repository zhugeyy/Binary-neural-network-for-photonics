# -*- coding: utf-8 -*-
"""
Binarized Neural Network (BNN) for MIR image classification
...
# ============================================================
# 3) Main Script (MIR image classification)
# ============================================================
...
# ----------------------------
# Dataset loader (MIR image)
# ----------------------------
def _list_class_folders(root_dir):
    """Return sorted class names by scanning subdirectories under `root_dir`."""
    names = []
...
def _load_images_bmp(folder, class_to_idx, target_size=(20, 20)):
    """
    Load grayscale BMP images from a class-folder dataset.

    Expected layout:
        folder/class_name/*.bmp

    Returns:
        X: image array of shape (N, H, W)
    ...
    if len(images) == 0:
        raise FileNotFoundError(
            f"No .bmp images found under: {folder}\n"
            "Expected: <folder>/<class_name>/*.bmp"
        )

    return np.stack(images), np.asarray(labels, dtype=np.int64)


def load_and_prepare_mir(data_dir, target_size=(20, 20)):
    """
    Load and preprocess the MIR dataset.

    Processing steps:
    1) read BMP images from the training/testing folders,
    2) resize them to `target_size`,
    3) flatten each image into a 1D vector,
    4) normalize pixel values to [0, 1],
    5) convert labels to one-hot targets in {-1, +1}.

    Returns:
        X_train, Y_train, X_test, Y_test, class_names
    """
    train_dir = os.path.join(data_dir, "training_image")
    test_dir = os.path.join(data_dir, "testing_image")

    if not os.path.isdir(train_dir) or not os.path.isdir(test_dir):
        raise FileNotFoundError(
            "Cannot find training_image/ or testing_image/ under --data.\n"
            f"Given --data: {data_dir}\n"
            f"Expected: {train_dir} and {test_dir}"
        )

    class_names = _list_class_folders(train_dir)
    if len(class_names) < 2:
        raise ValueError(
            f"Found {len(class_names)} class folder(s) under {train_dir}. "
            "Expected >= 2 classes."
        )
    ...
    X_train, y_train = _load_images_bmp(train_dir, class_to_idx, target_size=target_size)
    X_test, y_test = _load_images_bmp(test_dir, class_to_idx, target_size=target_size)

    # Flatten each resized image into a feature vector for the MLP.
    X_train = X_train.reshape(X_train.shape[0], -1).astype("float32") / 255.0
    X_test = X_test.reshape(X_test.shape[0], -1).astype("float32") / 255.0

    # Convert labels to one-hot format and remap them to {-1, +1}.
    Y_train = to_categorical(y_train, nb_classes) * 2 - 1
...
# Model builder
# ----------------------------
def build_bnn_mlp(input_dim=400, nb_classes=6, num_hidden=2, num_unit=100,
                  H="Glorot", kernel_lr_multiplier="Glorot",
                  use_bias=False, drop_in=0.05, drop_hidden=0.4,
                  epsilon=1e-6, momentum=0.9):
    """
    Build the fully-connected BNN used for MIR classification.
    """
    model = Sequential()
...
def train_and_export(model, X_train, Y_train, X_test, Y_test,
                     lr_start=1e-3, lr_end=1e-6, epochs=100, batch_size=32,
                     history_csv="training_history_mir.csv",
                     acc_fig="BNN_MIR_accuracy_curve.jpg",
                     loss_fig="BNN_MIR_loss_curve.jpg",
                     model_path="mlp_mir_cpu.h5",
                     weights_dir="weights_mir_csv",
                     cm_fig="confusion_matrix_mir.png",
                     class_names=None):
    """
    ...
    # Export each layer's weights to CSV for inspection or downstream use.
    ...
    plt.title("Confusion Matrix (MIR)")
    plt.tight_layout()
...
def main():
    # --------------------
    # Command-line arguments, define your own file path
    # --------------------
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--data",
        type=str,
        default=r"E:\MIR_classification",
    )
    ...
    # Data
    # --------------------
    X_train, Y_train, X_test, Y_test, class_names = load_and_prepare_mir(
        args.data,
        target_size=(20, 20),
    )

    # --------------------
    # Model definition (kept aligned with the MNIST version)
    # --------------------
    model = build_bnn_mlp(
        input_dim=400,
        nb_classes=len(class_names),
